...
//				return;
		...
		_addEvents : function() {
			...
			if (tinymce.isWebKit) {
				t.onClick.addToTop(function(ed, e) {
                    e = e.target;
		...
		addCommands({
			...
			RemoveFormat : function(command) {
                ...
                if(tinymce.isWebKit){
                    ...
                    var $tables = $j("table", tinymce.activeEditor.getBody());
                    $tables.css("display", "inline-table");
                    setTimeout(function(){$tables.css("display", "");}, 0);
                }
            },
...
(function(tinymce) {
	...
	tinymce.create('tinymce.ForceBlocks', {
		...
		setup : function() {
            ...
            // removed preprocessor that would add nbsp to empty inline elements
            // fix for JIVE-3817
		...
		function apply(name, vars, node) {
			...
			function applyRngStyle(rng) {
				...
				each(newWrappers, function(node) {
					...
					// Remove empty nodes
					if (childCount === 0 && !(node.parentNode && isBlockContainer(node.parentNode.nodeName.toLowerCase()))) {
						dom.remove(node, 1);
						return;
					}
