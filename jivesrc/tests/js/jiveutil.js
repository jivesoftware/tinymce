...
test('Formatter - remove format around link 2', function() {
	...
	equals(ed.getContent(), '<p>asdf <a href="javascript:;" jivemacro="user" ___default_attr="2001" _jivemacro_uid="_13056727656977177">Mr User 2</a> asdf</p>');
...
asyncTest('Formatter - remove format around link 5', function() {
    ...
    setTimeout(function(){
        ...
        ed.selection.setRng(rng);
        ed.execCommand("mceRemoveFullFormat");
        equals(ed.getContent(), '<p><span style="font-size: 18pt;">asdf </span><a href="javascript:;" jivemacro="user" ___default_attr="2001" _jivemacro_uid="_13056727656977177">Mr User 2</a><span style="font-size: 18pt;"><span> asdf</span></span></p>');
...
test("don't trim text inside tags", function() {
	ed.setContent('<p>first </p>');
	equals(ed.getContent(), '<p>first </p>');

    if(!tinymce.isIE){
        //
        // IE has a bug where whitespace at the beginning
        // of a paragraph is stripped out :(
        ed.setContent('<p> first</p>');
        equals(ed.getContent(), '<p> first</p>');
