...
test("Wiring, regexp, string and object literal callbacks", function(){
    ...
    ok(0 <= mi.render().firstChild.src.indexOf("/resources/scripts/tiny_mce3/tests/renderString"));
...
test("wiring", function(){
    ...
    this.contextPlugin.$menu.mouseenter();
    verify(mouseEnter, once())(anything());

    this.contextPlugin.$menu.mouseleave();
    ...
    this.contextPlugin.$menu.mouseenter(); //should be unbound by hide
    this.contextPlugin.$menu.mouseleave();
...
asyncTest("menu shows in img", function(){
    ...
    click(this.imgElem, function(){
        ok(that.contextPlugin.$menu.is(":visible"), "$menu is visible");
        ok(that.contextPlugin.rootMenu.isVisible(), "root menu should show");
        start();
    });
});
...
asyncTest("menu transition and rendering in list item", function(){
    ...
    click(this.firstLi, function(){
        ok(that.contextPlugin.rootMenu.isVisible(), "root menu should show");
        click(that.contextPlugin.$menu.find(".menuItems:visible :first").get(0), function(){
            equal(that.contextPlugin.$menu.find(".menuItems:visible").children().length, 4, "4 list style items visible");
            equal(that.contextPlugin.$menu.find(".menuItems:visible .menuItem_defaultListStyle").length, 1, "default list style item is visible");
            equal(that.contextPlugin.$menu.find(".menuItems:visible .menuItem_diListStyle").length, 1, "disc list style item is visible");
            equal(that.contextPlugin.$menu.find(".menuItems:visible .menuItem_cListStyle").length, 1, "circle list style item is visible");
            equal(that.contextPlugin.$menu.find(".menuItems:visible .menuItem_sListStyle").length, 1, "square list style item is visible");
            start();
        });
    });
});
