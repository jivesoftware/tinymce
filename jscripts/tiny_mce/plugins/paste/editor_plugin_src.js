...
//			console.log('Before preprocess:' + o.content);
                    ...
                    function wordStyleTransform(str, tag, style) {
                        ...
                        each(s, function wordStyleTransformInnerLoop(v) {
                            ...
                            if (parts.length == 2) {
                                ...
                                switch (name) {
                                    ...
                                    case "mso-vertical-align-alt":
                                        ...
                                        if(value != "0"){
                                            ...
                                            if(name == "margin-left" && $j.trim(lowerTag) == "<p"){
                                ...
                                if ((!styleProps.length || $j.inArray(name, styleProps)) >= 0 && parts[1] != "0" && parts[1] != "initial") {
            ...
            if(match != null){
                ...
                var lines = h.split("\n");
                var out = [];
                $j.each(lines, function(index, value){
                    out.push($j.trim(value));
                });
                h = out.join(" ");
            }
...
//			console.log('After preprocess:' + h);
		...
		_postProcess : function(pl, o) {
			...
			if (o.wordContent) {
                ...
                each(dom.select('*', o.node), function(el) {
                    if (el.nodeName == 'SPAN' && !el.className && !$j(el).attr("style")){
                        dom.remove(el, true);
                    }
                });
			}
			...
			if (getParam(ed, "paste_remove_styles") || (getParam(ed, "paste_remove_styles_if_webkit") && tinymce.isWebKit)) {
				...
				if (tinymce.isWebKit) {
					...
					each(dom.select('*', o.node), function(el) {
                        if($j(el).attr("_mce_style")) $j(el).attr("style", $j(el).attr("_mce_style"));
						el.removeAttribute('_mce_style');
					});
				}
			}
            ...
            //
            // make sure that every node is either
            // a block level node, or wrapped in a
            // paragraph.
            //
            ...
            if(o.node.childNodes.length > 1){
                ...
                for(var i=0;i<o.node.childNodes.length;i++){
                    var kid = o.node.childNodes[i];
                    var sib = kid.nextSibling;
                    if(kid.nodeType == 3 || !dom.isBlock(kid) && kid.nodeName.toLowerCase() != "br"){
                        if(sib && sib.nodeType == 1 && sib.nodeName.toLowerCase() == "p"){
                            if(sib.innerHTML.length == 0){
                                sib.parentNode.removeChild(sib);
                            }
                        }else if(sib && sib.nodeType == 1 && sib.nodeName.toLowerCase() == "br"){
                            sib.parentNode.removeChild(sib);
                            force = true;
                        }
                        kids.push(kid);
                    }else if(kid.nodeType == 1 && kid.nodeName.toLowerCase() == "br"){
                        var p = kid.ownerDocument.createElement("p");
                        kid.parentNode.insertBefore(p, kid);
                        p.appendChild(kid);
                        kid = p;
                    }
                    if(sib && sib.nodeType == 1 && dom.isBlock(sib) || force){
                        if(kids.length){
                            i = i - kids.length + 1;
                            var p = kids[0].ownerDocument.createElement("p");
                            o.node.insertBefore(p, kids[0]);
                            do{
                                var spl = kids.splice(0,1);
                                p.appendChild(spl[0]);
                            }while(kids.length);
                        }
                        kids = new Array();
            ...
            $j(o.node).find("p").each(function(){
                var par = $j(this);
                $j(this).find("p").each(function(){
                    var childP = $j(this);
                    ed.dom.split(childP.parents("p").get(0),childP.get(0));
                });
            });
            ...
            // JIVE-599
            $j(o.node).find("a.jive_macro").each(function(){
                var a= $j(this);
                if(!a.attr("href")){
            ...
            if(ed.plugins.jiveimage){
                each(dom.select('img', o.node), function(img) {
                    ...
                    }else if(img.src.substr(0, 5) == "file:"){
                        //file URLs don't post right, so let's not tease the user
                        dom.remove(img);
                    }
                });

                setTimeout(function(){
                    each(dom.select('img.toUpload', ed.getBody()), function(img) {
                        dom.removeClass(img, "toUpload");
                        var pasteFilename = dom.uniqueId("pastedImage_");
                        ed.plugins.jiveimage.uploadDataUrlImage(pasteFilename, img);
                    });
                }, 100);
            }
		...
		_convertLists : function(pl, o) {
            ...
            function getParagraphMetaData(p) {
                ...
                var forceType = null;
                if($j(p).children("span[jive-list-ol=1]").length){
                    forceType = "ol";
                }
                if($j(p).children("span[jive-list-ul=1]").length){
                    forceType = "ul";
                }
